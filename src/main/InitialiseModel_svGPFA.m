function m = InitialiseModel_svGPFA(lik,link,Y,trLen,kerns,Z,prs,options)
...
%
% link        -- string taking values {'Exp','ReLu'} specifying which link
%                function to use
%
...
%
%
% See also: variationalEM, buildKernel, InitialiseModel_grouped_svGPFA
%
%
% Duncker, 2017
%
...
m.epsilon = 1e-02; % value of diagonal added to kernel inversion for stability
m.dx = size(prs.C,2);
...
% sample minibatches for each number of SGD
m = m.EMfunctions.sampleMinibatch(m);

% save Gauss-Hermite Quadrature nodes and weights
[xxHerm,wwHerm] = hermquad(m.opts.nquad_link); 
m.xxHerm = xxHerm;
m.wwHerm = wwHerm/sqrt(pi);
% set other likelihood dependent things
switch lik
        
    case 'Poisson'
        % arrange data into appropriate format
        [Ystack,trLenN,mask,tsamp,BinWidth] = arrangeData_Poisson(m,Y,trLen);
    ...
    case 'PointProcess'
        % arrange data into appropriate format
        [Ystack,index] = arrangeData_PointProcess(m,Y);
